...
func on_score_changed(new_score: int) -> void:
	%Score.set_deferred("text", "%d" % new_score)

func on_lives_changed(new_lives: int) -> void:
	%Lives.set_deferred("text", "%d" % new_lives)
...
func handle_pop(bubble: Bubble) -> void:
	...
	if bubble.stats.value > 0:
		...
		self._score_twn.tween_property(%Score, "modulate", Color.WHITE, 1)
...
func handle_escape(bubble: Bubble) -> void:
	...
	%Lives.text = "LIVES: %d" % GameService.lives
	...
	self._lives_twn.tween_property(%Lives, "modulate", Color.WHITE, 1)
